    ...
    reader.onload = (e) => {
      try {
        ...
        for (let cellAddress in worksheet) {
          if (cellAddress[0] === '!') continue; // 跳过元数据
          ...
          if (row && row.length > 0) {
            ...
            headers.forEach((header, index) => {
              if (header && row[index] !== undefined) {
                ...
                // 检查是否有对应的超链接
                const cellAddress = XLSX.utils.encode_cell({c: index, r: i});
                if (links[cellAddress]) {
                  productLinks[header] = links[cellAddress];
                }
              }
            });
            ...
            if (product['ASIN']) {
              products.push(formatProductData(product, productLinks));
            }
          }
        }
 ...
 * @param {Object} productLinks - 产品相关的超链接信息
 * @returns {Object} 格式化后的产品数据
 */
const formatProductData = (rawProduct, productLinks = {}) => {
  // 生成小类目URL
  const generateSubCategoryUrl = (categoryPath, subCategory, mainCategory) => {
    if (!subCategory) return '';
    
    try {
      // 创建搜索URL，在特定类目下搜索小类目
      const cleanSubCategory = subCategory.replace(/[&'",]/g, '').trim();
      const searchQuery = encodeURIComponent(cleanSubCategory);
      
      // 根据主类目选择搜索范围
      let categoryFilter = '';
      const mainCatLower = mainCategory ? mainCategory.toLowerCase() : '';
      
      if (mainCatLower.includes('kitchen') || mainCatLower.includes('dining')) {
        categoryFilter = '&rh=n%3A284507'; // Kitchen & Dining node
      } else if (mainCatLower.includes('home') || mainCatLower.includes('garden')) {
        categoryFilter = '&rh=n%3A1055398'; // Home & Garden node
      } else if (mainCatLower.includes('pet')) {
        categoryFilter = '&rh=n%3A2619533011'; // Pet Supplies node
      } else if (mainCatLower.includes('sports') || mainCatLower.includes('outdoor')) {
        categoryFilter = '&rh=n%3A3375251'; // Sports & Outdoors node
      } else if (mainCatLower.includes('toy')) {
        categoryFilter = '&rh=n%3A165793011'; // Toys & Games node
      } else if (mainCatLower.includes('automotive')) {
        categoryFilter = '&rh=n%3A15684181'; // Automotive node
      }
      
      return `https://www.amazon.com/s?k=${searchQuery}${categoryFilter}&ref=sr_nr_n_1`;
    } catch (error) {
      console.warn('生成小类目URL失败:', error);
    }
    return '';
  };
  ...
  return {
    ...
    // 超链接信息
    mainCategoryUrl: productLinks['大类目'] || '',
    subCategoryUrl: generateSubCategoryUrl(rawProduct['类目路径'], rawProduct['小类目'], rawProduct['大类目']),
    // 保留原始数据以备后用
    raw: rawProduct,
    links: productLinks
  };
